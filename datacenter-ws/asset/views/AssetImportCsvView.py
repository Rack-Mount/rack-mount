...
from asset.utils.asset_import import (
    TEMPLATE_HEADERS,
    REQUIRED_COLUMNS,
    ModelLookupCache,
    int_or_none,
...
class AssetImportCsvView(APIView):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = (
            'attachment; filename="asset_import_template.csv"'
        )
        return response
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                description='Import result with created count and per-row errors'
            ),
    ...
    def post(self, request):
        ...
        for row_num, row in enumerate(reader, start=2):   # row 1 = header
            try:
                self._import_row(row, row_num, lookup, rows)
                created += 1
            except ValueError as exc:
                logger.warning(
                    "Errore di validazione CSV riga %s: %s", row_num, exc
                )
                errors.append(
                    {
                        'row': row_num,
                ...
                logger.exception(
                    "Errore imprevisto CSV riga %s", row_num
                )
                errors.append(
                    {
                        'row': row_num,
        ...
        return Response(
            {'created': created, 'rows': rows, 'errors': errors},
    ...
    def _import_row(
        ...
        """Validate and create one Asset from a CSV row dict.

        Appends the result to *rows* on success.
        Raises :class:`ValueError` for any validation failure.
        """
        ...
        serial_number = _sanitize_cell((row.get('Seriale') or '').strip())
        ...
        state_obj = lookup.get_state(state_name)
        ...
        warranty_exp = parse_date(row.get('Scad. garanzia', ''))
        support_exp = parse_date(row.get('Scad. supporto', ''))
        purchase_date = parse_date(row.get('Data acquisto', ''))
        decommission_date = parse_date(row.get('Data dismissione', ''))
